  ...
  connection.query(
    ...
    function (err, results, fields) {
      ...
      for (let i = 0; i <= results.length; i++) {
        ({ nombre, precio, total } = results[0][i]);
        productos += `<li> Nombre producto: ${nombre} / Precio: ${precio} / ${total} left </li>`;
      }
...
app.get("/masGasto", function (request, response) {
  ...
  //Pedir listado de producto desde la base de dato
  connection.query(
    "select sum(cantidad) as compra, nombreUS, id_usuarios from ventas inner join usuarios on ventas.id_usuarios = usuarios.id group by nombreUS order by compra desc limit 3;",
    function (err, results, fields) {
      if (err) {
        return console.error("error:" + err.message);
      }
      //nombre, total y ...rest, sacar por consola ...rest
      let usuarios = "";
      for (let i = 0; i < results.length; i++) {
        ({ nombreUS, compra, ...rest } = results[i]);
        console.log(rest);
        usuarios += `<li>  Nombre: ${nombreUS} / Total compra: ${compra} / id: ${rest.id_usuarios}  </li>`;
      }
...
app.get("/usuarioMasGasto", function (request, response) {
  ...
  //Pedir listado de producto desde la base de dato
  connection.query(
    "select sum(cantidad * precio) as Gasto , nombreUS, cantidad, precio from ventas inner join usuarios on ventas.id_usuarios = usuarios.id group by nombreUS order by Gasto desc limit 3;",
    function (err, results, fields) {
      if (err) {
        return console.error("error:" + err.message);
      }
      ...
      for (let i = 0; i < results.length; i++) {
        ({ nombreUS, Gasto, ...resto } = results[i]);
        console.log(resto);
        usuariosMasGasto += `<li> Nombre: ${nombreUS} / Gasto: ${Gasto}€ / Cantidad: ${resto.cantidad} / Precio: ${resto.precio}€ </li>`;
      }
